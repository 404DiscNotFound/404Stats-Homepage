  ...
  return (
    <div id="how" className="relative z-10 mx-auto max-w-5xl px-6 py-16">
      <div className="mb-12 text-center">
        ...
        <p className="mt-2 text-sm text-[#888888]">{t("landing.how.desc")}</p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        {STEPS.map(({ icon: Icon, num, key }) => (
          <div key={key} className="group rounded-xl border border-[#3D3D3D] bg-[#2E2E2E] p-6 transition-all hover:border-[#5BA033]/30 hover:shadow-[0_0_20px_rgba(91,160,51,0.06)]">
            <div className="flex items-center gap-3">
              <Icon className="h-5 w-5 text-[#5BA033]/40" />
              <span className="text-2xl font-black text-[#3D3D3D] transition-colors group-hover:text-[#5BA033]/30">{num}</span>
              <ChevronRight className="h-4 w-4 text-[#444444]" />
            </div>
            <h3 className="mt-3 text-sm font-bold text-white">{t(`landing.how.${key}Title`)}</h3>
            <p className="mt-1 text-xs leading-relaxed text-[#888888]">{t(`landing.how.${key}Desc`)}</p>
          </div>
        ))}
      </div>
      <div className="mt-12">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-xs text-[#888888]">{t("landing.commands.desc")}</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          {COMMANDS.map((key) => (
            <div key={key} className="flex items-center gap-4 rounded-lg border border-[#3D3D3D] bg-[#2E2E2E] p-4">
              <code className="shrink-0 rounded bg-[#5BA033]/10 px-2 py-1 text-xs font-bold text-[#5BA033]">{t(`landing.commands.${key}`)}</code>
              <span className="text-xs text-[#888888]">{t(`landing.commands.${key}Desc`)}</span>
            </div>
          ))}
        </div>
      </div>
