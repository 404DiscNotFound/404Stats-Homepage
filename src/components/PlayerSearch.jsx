    ...
    <div className="relative">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-[#1A1A24] bg-[#0A0A0F] py-2.5 pl-10 pr-4 text-sm text-white placeholder-gray-600 outline-none focus:border-[#00F5FF]/50"
        />
      </div>
      {focused && filtered.length > 0 && (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-lg border border-[#1A1A24] bg-[#0F0F14] shadow-xl">
          {filtered.map(p => (
            <button
              key={p.uuid}
            ...
            >
              <img
                src={`https://crafatar.com/avatars/${p.uuid}?size=32`}
                alt=""
