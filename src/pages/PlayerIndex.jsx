...
const SORT_TABS = [
  { key: "total", labelKey: "blockIndex.sortTotal" },
  { key: "mined", labelKey: "blockIndex.sortMined" },
  { key: "placed", labelKey: "blockIndex.sortPlaced" },
  { key: "name", labelKey: "playerIndex.sortName" },
];
      ...
      <div className="relative z-10">
        ...
        <div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 sm:py-8">
          ...
          <div className="mb-4 flex items-center justify-between sm:mb-6">
            <div>
              <h1 className="text-lg font-black text-white sm:text-2xl">{t("playerIndex.title")}</h1>
              <p className="text-xs text-gray-600 sm:text-sm">
                {data?.totalPlayers || 0} {t("dashboard.players")} · {formatNumber(data?.totals?.combined || 0)} {t("blockIndex.totalActions")}
              </p>
            </div>
            <Link
              to={`/server/${slug}`}
          ...
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={t("playerIndex.searchPlaceholder")}
          ...
          {players.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-gray-600">
                {search ? t("playerIndex.noResults") : t("playerIndex.noData")}
              </p>
            </div>
          ) : (
            <>
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-center gap-4">
                  ...
                  >
                    <ChevronLeft className="h-3.5 w-3.5" /> {t("playerIndex.prev")}
                    </button>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg border border-[#1A1A24] bg-[#0A0A0F] px-3 py-2 text-xs font-bold text-gray-400 transition-all hover:border-[#00F5FF]/30 hover:text-[#00F5FF] disabled:opacity-30 disabled:hover:border-[#1A1A24] disabled:hover:text-gray-400"
                  >
                    {t("playerIndex.next")} <ChevronRight className="h-3.5 w-3.5" />
                  </button>
                </div>
              )}
            </>
