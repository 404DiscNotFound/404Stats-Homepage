          ...
          <aside className="lg:w-64 lg:shrink-0">
            ...
            <div className={`${mobileSidebarOpen ? "block" : "hidden"} lg:block`}>
              ...
              <div className="relative mb-4">
                <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#5A5A5A]" />
                <input
                  type="text"
                  value={searchQuery}
              ...
              {searchResults ? (
                <div>
                  <p className="mb-2 px-1 text-[10px] uppercase tracking-widest text-[#5A5A5A]">
                    {searchResults.length} {t("wiki.results")}
                  </p>
                  {searchResults.length === 0 ? (
                    <p className="px-1 text-xs text-[#5A5A5A]">{t("wiki.noResults")}</p>
                  ) : (
                  ...
                  {CATEGORIES.map((cat) => (
                    <div key={cat.id}>
                      <div className="mb-1.5 flex items-center gap-2 px-1">
                        {cat.icon && <cat.icon className="h-3.5 w-3.5 text-[#5A5A5A]" />}
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A5A]">{t(cat.labelKey)}</span>
                      </div>
